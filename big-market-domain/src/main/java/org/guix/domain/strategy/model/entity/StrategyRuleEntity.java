...
public class StrategyRuleEntity {
    ...
    public Map<String, List<Integer>> getRuleWeightValues() {
        ...
        String[] ruleValueGroups = ruleValue.split(Constants.SPACE);//分成4000 5000 6000三组
        ...
        for (String ruleValueGroup : ruleValueGroups) {
            ...
            // 分割字符串以获取键和值
            String[] parts = ruleValueGroup.split(Constants.COLON);//根据冒号欺负冒号前（权重值）冒号后（可抽奖的序号）
            if (parts.length != 2) {
                throw new IllegalArgumentException("rule_weight rule_rule invalid input format" + ruleValueGroup);
            }
            // 解析值
            String[] valueStrings = parts[1].split(Constants.SPLIT);//可抽奖的序号依次装填
